...
void USART3_IRQHandler(void) 
{
	...
	if(USART_GetITStatus(USART3,USART_IT_RXNE)!=RESET)
	{
		...
		else	if (Bluetooth_data==0x09)
		{
		  int RIGHT = GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_5);
		  int LEFT = GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_6);
			if (LEFT==1)Fore=0,Back=0,Left=1,Right=0;
			else if (RIGHT==1)Fore=0,Back=0,Left=0,Right=1;
			else Fore=1,Back=0,Left=0,Right=0;
   	}
