...
//		OLED_ShowString(0,0,uart_buf,16);
		...
		else if(Bluetooth_data==0x01)	
		{
			while(1)
			{
	if(xunjiR_Get_State() == 0 && xunjiL_Get_State() == 0) //2个探头均未压线，直行
	{
		Fore=0,Back=1,Left=0,Right=0;
	}
	else if(xunjiR_Get_State() == 1 && xunjiL_Get_State() == 0) //左探头压线，车子右倾，左转
	{
		Fore=0,Back=0,Left=1,Right=0;//左转
	}
	else if(xunjiR_Get_State() == 0 && xunjiL_Get_State() == 1) //右探头压线，车子左倾，右转
	{
		Fore=0,Back=0,Left=0,Right=1;//右转
	}
	else if(xunjiR_Get_State() == 1 && xunjiL_Get_State() == 1) //2个探头均压线，停止
	{
		Fore=0,Back=0,Left=0,Right=0;//停车
	}
	else if(Bluetooth_data==0x09) break;
    }
}
